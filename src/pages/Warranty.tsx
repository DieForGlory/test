...
export function Warranty() {
  ...
  useEffect(() => {
    const loadPolicy = async () => {
      try {
        const result = await publicApi.getPolicy('warranty');
        setData(result);
      } catch (error) {
        console.error('Error loading warranty policy:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Если данных нет, ничего не рендерим или можно показать заглушку
  if (!data) return null;
